...
class connect4():
    ...
    def gameOver(self, j, player):
            ...
            if count == 4:
                if self.visualize:
                    pygame.draw.line(screen, WHITE, (int(s * SQUARESIZE + SQUARESIZE / 2), int((i + 1.5) * SQUARESIZE)),
                                     (int((s - 4) * SQUARESIZE + SQUARESIZE + SQUARESIZE / 2),
                                      int((i + 1.5) * SQUARESIZE)), 5)
        ...
        for s in range(maxColumnIndex, minColumnIndex + 1):
            ...
            if count == 4:
                if self.visualize:
                    pygame.draw.line(screen, WHITE, (int(j * SQUARESIZE + SQUARESIZE / 2), int((s + 2) * SQUARESIZE)),
                                     (int(j * SQUARESIZE + SQUARESIZE / 2), int((s - 2) * SQUARESIZE)), 5)
                    pygame.display.update()
                return True
        # Left diagonal
        row = i
        ...
        if count >= 4:
            if self.visualize:
                ...
                pygame.draw.line(screen, WHITE, (
                    int((j + 0.5 - (down_count - 1)) * SQUARESIZE), int((i + 1.5 - (down_count - 1)) * SQUARESIZE)), (
                                     int((j + 0.5 + (4 - down_count)) * SQUARESIZE),
                                     int((i + 1.5 + (4 - down_count)) * SQUARESIZE)), 5)
                pygame.display.update()
            return True
        # Right diagonal
        row = i
            ...
            for r in range(self.shape[0]):
                if self.board[r][c] == 1:
                    pygame.draw.circle(screen, RED, (
                        int((c) * SQUARESIZE + SQUARESIZE / 2), height - int((5 - r) * SQUARESIZE + SQUARESIZE / 2)),
                                       RADIUS)
